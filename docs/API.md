...
Actors: `PUT /api/v1/maize/actors/{slug}` writes `$COOG_DATA_PATH/people/…/actor.meta.json` (`name`, `bio`, `birthday`, `birthplace`, `ethnicity`, `height`, `measurements`, `yearsActive`, `aliases`, `links`, `locked`). Creates the folder if missing. Manual saves default `locked` to true. `POST …/headshot` stores Jellyfin-style `folder.jpg|png|webp`.
